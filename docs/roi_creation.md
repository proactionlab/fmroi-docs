...
0. Select the source image by clicking its name in the [table of loaded images](#selecting-the-source-image);
1. Select the ROI algorithm from the Method dropdown menu;
2. Check if the the image that you selected is shown in the *sorce image* fiels;
3. Select wether the ROI size will be defined by the radius (number of voxel in the radius) or volume (total number of voxels into the ROI). Note that when selecting the ROI size by its volume, spheremask will try to create a ROI as similar as possible to a sphere, however it possibly will be less "spherical" then the ROIs generated by the *radius* method.
4. Enter the number of voxels to define the ROI size.
